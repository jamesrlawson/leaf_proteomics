...
Amax <- climate_locs %>% select(photo_max, leaf_age, biological_rep, ID) %>%
                         filter(!is.na(photo_max)) %>%
                         group_by(ID, leaf_age) %>%
                         summarise(photo_max = mean(photo_max)) %>%
                         ungroup()

Amax$Amax_stand <- NA

for(i in unique(Amax$ID)) {
 
  Amax[Amax$ID %in% i,]$Amax_stand <- Amax[Amax$ID %in% i,]$photo_max / subset(Amax[Amax$ID %in% i,], leaf_age == 'new')$photo_max
   
}

Amax$leaf_age <- factor(Amax$leaf_age, levels = c("new", "mid", "old"))

boxplot(Amax_stand ~ leaf_age, Amax, xlab = 'leaf_age', ylab = 'standardised Amax')

boxplot(photo_max ~ leaf_age, Amax, xlab = 'leaf_age', ylab = 'unstandardised Amax')
