...
regression_output_agg <- function(df, indepvar, logx) {
  ...
  for(i in 1:ncol(df)) {
    ...
    model.stats <-  data.frame(cbind(protname,round(model$r.squared,3),
                                     round(model$coefficients[,4][2],2), length(unique(df[[protname]]))))
    
    names(model.stats) <- c('submodel','R2','pval', 'n') 
...
aggplot_save_combined_4 <- function(proportion, indepvar, logx = FALSE, indepvarType, labs,
                                    save=TRUE, outDir, fileType = 'tiff', 
                                    output_list = FALSE, goldenRatio = FALSE, yaxis = FALSE) {
  ...
  if(logx) { # get significance TRUE/FALSE for model, depending on if logx or not
    model_calv <- lm(dep_means[dep_means$funccat %in% 'calv',]$protein_mean ~ log10(dep_means[dep_means$funccat %in% 'calv',][[indepvar]]))
    model_phot <- lm(dep_means[dep_means$funccat %in% 'phot',]$protein_mean ~ log10(dep_means[dep_means$funccat %in% 'phot',][[indepvar]]))
    sig_calv <- summary(model_calv)$coefficients[,4][2] < 0.05
    sig_phot <- summary(model_phot)$coefficients[,4][2] < 0.05
  } else {
    model_calv <- lm(dep_means[dep_means$funccat %in% 'calv',]$protein_mean ~ dep_means[dep_means$funccat %in% 'calv',][[indepvar]])
    model_phot <- lm(dep_means[dep_means$funccat %in% 'phot',]$protein_mean ~ dep_means[dep_means$funccat %in% 'phot',][[indepvar]])
    sig_calv <- summary(model_calv)$coefficients[,4][2] < 0.05
    sig_phot <- summary(model_phot)$coefficients[,4][2] < 0.05
  }
  ...
  # output lm summaries to list
  
  outlist[[1]] <- model_calv
  outlist[[2]] <- model_phot
  ...
  if(output_list){return(outlist)}
  
}

# generate bespoke lm outputs
# takes a list where element 1 is the lm for calvin cycle vs whatever indepvar, element 2 is the lm for photosystems vs indepvar
lmSummary <- function(l, logx) {
  
  # logx = FALSE
  
  lm_summary <- glance(summary(l[[1]])) %>%
    rbind(., glance(summary(l[[2]])))
  lm_summary$df[1] <-  summary(l[[1]])$df[2]
  lm_summary$df[2] <-  summary(l[[2]])$df[2]
  
  lm_summary <- cbind(model = c('light independent rxns',
                                'photosystems'),
                      lm_summary)
  lm_summary$adj.r.squared <- NULL
  
  # light independent rxns model
  
  mod <- l[[1]]
  
  if(logx) {
    start = as.numeric(coef(mod)[1]) + as.numeric(coef(mod)[2]) * min(mod$model[2])
    end = as.numeric(coef(mod)[1]) + as.numeric(coef(mod)[2]) * max(mod$model[2])
  } else { 
    if(!logx) {
      start = as.numeric(coef(mod)[1]) + as.numeric(coef(mod)[2]) * min(mod$model[2])
      end = as.numeric(coef(mod)[1]) + as.numeric(coef(mod)[2]) * max(mod$model[2])
  ...
  lm_summary$fit_start <- NA
  lm_summary$fit_end <- NA
  lm_summary$fit_change <- NA
  
  lm_summary$fit_start[1] <- start
  lm_summary$fit_end[1] <- end
  lm_summary$fit_change[1] <- (end - start)/start
  ...
  lm_summary$fit_start[2] <- start
  lm_summary$fit_end[2] <- end
  lm_summary$fit_change[2] <- (end - start)/start
