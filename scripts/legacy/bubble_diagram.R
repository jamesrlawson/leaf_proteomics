### this script produces output used to create the first 'field vs glasshouse photosynthesis protein amounts' bubble diagram
...
protein_stand_D14[,2:26] <- protein_stand_D14[,2:26]/protein_stand_D14$total_protein
rm(protein_bins_D14_spread)

# relative quants, standardised by Rubisco

protein_RbcStand_D14 <- protein_bins_D14[,c('sample', 'bin_arch_name','sum')]
protein_RbcStand_D14 <- na.omit(protein_RbcStand_D14)
protein_RbcStand_D14 <- spread(protein_RbcStand_D14, key = bin_arch_name, value=sum)
protein_RbcStand_D14[,2:26] <- protein_RbcStand_D14[,2:26]/protein_RbcStand_D14$Rubisco

protein_RbcStand_D14 <- ddply(melt(protein_RbcStand_D14[,c('Rubisco','Calvin_cycle','Cytochrome_b6f','PSI','PSII','ATP_synthase_chloroplastic', 'electron_transport_minATPsynth')]), 
                              .(variable), 
                              summarise,
                              mean = mean(value, na.rm=TRUE),
                              CV = CV(value))