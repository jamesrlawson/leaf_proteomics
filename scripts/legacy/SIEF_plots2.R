...
data_means <- bla %>% group_by(ID) %>% summarise(Rubisco_mean = mean(Rubisco, na.rm=TRUE),
                                                 ...
                                                 rbact_per_PRK_mean = mean((rb_act/eucgr.e01261.1.p), na.rm=TRUE),
                                                 rbact_per_PRK_SE = SE(rb_act/eucgr.e01261.1.p),
                                                 Calv_ex_rbact_mean = mean((Calvin_cycle - Rubisco), na.rm=TRUE),
                                                 Calv_ex_rbact_SE = SE(Calvin_cycle - Rubisco),
                                                 PGLP_per_PGK_mean = mean(eucgr.b01439.1.p/eucgr.f01476.1.p, na.rm=TRUE),
                                                 PGLP_per_PGK_mean = SE(eucgr.b01439.1.p/eucgr.f01476.1.p)) %>%
  full_join(bla, by = 'ID') %>%
...
# rbact per PRK vs tavg

model <- summary(lm(rbact_per_PRK_mean ~ tavg, data_means))
...
p <- ggplot(data_means, aes(y = rbact_per_PRK_mean, x = tavg)) 
p <- p + geom_point(size = 4) 
p <- p + geom_smooth(method = 'lm', size=0.5, colour = 'black', se=FALSE)
p <- p + geom_errorbar(aes(ymin = rbact_per_PRK_mean - rbact_per_PRK_SE, ymax = rbact_per_PRK_mean + rbact_per_PRK_SE), width = 0.3)
p <- p + expand_limits(y=0)
p <- p + ylab('Rbc_act per PRK abundance') + xlab(expression(paste("Mean annual temperature (",degree,"C)", sep = "")))
p <- p + theme_bw()
...
# PGLP vs PGK colored by T.

model <- summary(lm(PGLP_mean ~ PGK_mean, data_means))
...
p <- p + expand_limits(y=0)
p <- p + ylab('Mean PGLP abundance (mg/m2)') + xlab('Mean PGK abundance (mg/m2)')
p <- p + theme_bw()
p <- p + theme(panel.grid.major = element_blank(),
               panel.grid.minor = element_blank(), 
               legend.position="top",
...
y = as.numeric(coef(model)[1]) + as.numeric(coef(model)[2]) * min(data_means$PGK_mean)
x = as.numeric(coef(model)[1]) + as.numeric(coef(model)[2]) * max(data_means$PGK_mean)

rise = x - y 

run = max(data_means$PGK_mean) - min(data_means$PGK_mean)

rise/run


# rbact per PRK colored by T. 

model <- summary(lm(rbact_mean ~ PRK_mean, data_means))
...
tiff(paste(outDir, '/', depvar, '_vs_', indepvar, '_R2-', round(model$r.squared,3), '_pval-', round(model$coefficients[,4][2],2),'.tiff', sep = ""), height = 6, width = 6, units = 'in', res = 600)
p <- ggplot(data_means, aes(y = rbact_mean, x = PRK_mean)) 
p <- p + geom_point(size = 4, aes(colour = tavg)) 
p <- p + scale_colour_gradientn(colours=c('cyan', 'red'), name = expression(paste("Mean annual temperature (",degree,"C)", sep = "")))
p <- p + geom_smooth(method = 'lm', size=1, colour = 'blue', se=FALSE)
...
p <- p + expand_limits(y=0)
p <- p + ylab('Mean rbact abundance (mg/m2)') + xlab('Mean PRK abundance (mg/m2)')
p <- p + theme_bw()
p <- p + theme(panel.grid.major = element_blank(),
               panel.grid.minor = element_blank(), 
               legend.position="top",
...
plot(PGLP_per_PGK_mean ~ tavg, data_means)
abline(lm(PGLP_per_PGK_mean ~ tavg, data_means))
summary(lm(PGLP_per_PGK_mean ~ tavg + log10(prec), data_means))


plot((eucgr.b01439.1.p / eucgr.f01476.1.p) ~ tavg, data_means)

data_means_naomit <- na.omit(data_means)

bla <- varpart(data_means_naomit$PGLP_per_PGK_mean,
               ~log10(prec),
               ~tavg,
               ~leaf_rad,
            #   ~d13C,
            #   ~alpha,
               data = data_means_naomit)
bla
plot(bla)
...
bla <- varpart(data_means$PGLP_mean,
               ~PGK_mean,
               ~tavg,
               ~log10(prec),
               data = data_means)
...
bla <- lm(scale(PGLP_mean) ~ scale(PGK_mean) + scale(tavg) + scale(log10(prec)), data_means)
summary(bla)
